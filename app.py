...
# Make a Dash app!
from fintech_ibkr.synchronous_functions import fetch_contract_details_new, place_order, fetch_current_time

app = dash.Dash(__name__)
server = app.server

# Define the layout.
df = pd.read_csv('submitted_orders.csv', on_bad_lines='skip')
app.layout = html.Div([
    ...
    html.H4("Enter a DurationStr:"),
    html.Div(
        dcc.Input(
            placeholder='Enter a value...',
            type='text',
            value='30 D', id='duration'
        )

    ),

    html.H4("Enter a barSizeSetting:"),
    html.Div(
        dcc.Input(
            placeholder='Enter a value...',
            type='text',
            value='1 hour', id='bar-size'
        )
...
@app.callback(
    ...
    [State('currency-input', 'value'), State('what-to-show', 'value'),
     ...
     State('duration', 'value'), State('bar-size', 'value'),
     State('use-RTH', 'value'), ]
)
def update_candlestick_graph(n_clicks, currency_string, what_to_show,
    ...
    cph = fetch_historical_data(
        ...
        durationStr=duration,  # <-- make a reactive input
...
@app.callback(
    ...
    [State('buy-or-sell', 'value'), State('contract-currency-input', 'value'), State('trade-amt', 'value'),
     State('mkt-or-lmt', 'value'),
